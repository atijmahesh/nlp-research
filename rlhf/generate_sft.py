...
"""
generate_sft.py
Generate completions using the SFT fine-tuned model.

Generates 250 samples per occupation using the fine-tuned checkpoint.
Output: sft_llama3_completions.csv
"""
...
# ─── CONFIGURATION ───────────────────────────────────────────────────────────
MODEL_PATH = "./sft_llama3_8b_output"  # Path to fine-tuned model
OUTPUT_CSV = "sft_llama3_completions.csv"
...
def main():
    ...
    if not Path(MODEL_PATH).exists():
        ...
        print("Please run train_sft.py first to train the model.")
        return
